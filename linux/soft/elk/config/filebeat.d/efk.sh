...
tomcat_pid=`ps -ef | grep [u]sr/local/tomcat | awk '{print $2}'`

efk_start(){
case $2 in
  elastic)
    [ -n "$elastic_pid" ] && echo "进程存在" && exit || echo "开始进程" && cd /usr/local/efk/alone/ && su lin -s /bin/bash -c "/usr/local/efk/alone/es/bin/elasticsearch -d &>>/var/log/elasticsearch/elasticsearch.log &";;
  elastic_head)
  ...
  tomcat)
    [ ! -z "$tomcat_pid" ] && echo "进程已存在" || /usr/local/tomcat/bin/startup.sh;;
  *)
    echo "实例名称为elastic/elastic_head/kibana/logstash/filebeat/tomcat";;  
esac
}
...
kill $pid &>> /dev/null && echo  "进程已杀死" || echo -e "\033[33m$1\033[0m实例未启动"
}

efk_status(){
echo "进程pid:" && echo "    elastic:${elastic_pid:="未启动"} elastic_head:${elastic_headpid:="未启动"} kibana${kibana_pid:="未启动"} logstash:${logstash_pid:="未启动"} filebeat:${filebeat_pid:="未启动"} tomcat:${tomcat_pid:="未启动"}" 
...
case $1 in
  start)
    [ $# == 3 ] && efk_start $1 $2 $3 && exit || echo "参数不够" && /usr/bin/echo '第一个参数是启动/停止。第二个参数是实例，第三个参数是配置文件/0';;
  stop)
    efk_stop $2;;
  status)
    efk_status;;
